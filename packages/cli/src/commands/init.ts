...
export function cmdInit(name: string, rootDir: string): { files: string[]; configPath: string } {
  ...
  // Install ambient type definitions for zero-config IDE TypeScript support
  installAmbientTypes(agentDir);
...
function installAmbientTypes(agentDir: string): boolean {
  const typesPath = path.join(agentDir, "jaxx.d.ts");
  if (fs.existsSync(typesPath)) return false;
  const typesContent = `// Ambient type declarations for Agent Jaxx Model
// Automatically generated by jaxx init to provide zero-config TypeScript support in IDEs.
